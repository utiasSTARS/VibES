...
def compensation_vis(events, frequency, amplitude_x, amplitude_y, phase_shift, time_window, width,
                     height):
    image = torch.zeros((height, width), dtype=torch.float64)
...
def objective_function(params, events, time_window, width, height):
    ...
    # sharpness = np.array(compensated_image_smooth).var()
...
if __name__ == "__main__":
    ...
    initial_guess = [20, 15, 45.0, 0, np.pi / 4]  # Adjust these based on your expectations
