...
SCRIPT_PATH=${2:-"/path/to/your/spread_meas.py"}
...
for file in "$DATASET_DIR"/events_*.hdf5; do
    echo "Launching spread for $file ..."
    python3 "$SCRIPT_PATH" "$file" --output "${file%.hdf5}_res.pkl" --downsample $DOWNSAMPLE_RATE &
...
echo "All spread computations finished ✅"