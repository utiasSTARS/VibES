...
dv::EventStore compensate(const dv::EventStore &events, float A, float f, float phi, const cv::Size &res) {
    ...
    dv::EventStore compensated_events;
    for (const auto &e: events) {

        float c = A * std::cos(f * (e.timestamp() - start_time) / 1e6 + phi);
        auto new_e = dv::Event(e.timestamp(), e.x() - c, e.y() - c, e.polarity());
        if (new_e.x() < 0 || new_e.x() >= res.width || new_e.y() < 0 || new_e.y() >= res.height) {
            continue;
        }
        compensated_events.push_back(new_e);
    }
