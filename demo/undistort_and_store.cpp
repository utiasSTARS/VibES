...
// unidistortion
    ...
    int width = camera.geometry().width();
    int height = camera.geometry().height();
    ...
    cd_stage_ptr->set_consuming_callback([&hdf5_writer, undistort, width, height](const boost::any &data) {
        try {
            auto buffer = boost::any_cast<Metavision::Stage::EventBufferPtr>(data);
            // were to copy the new events
            std::vector<Metavision::EventCD> events;
            for (auto &ev: *buffer) {
                auto [x, y] = undistort(ev.x, ev.y);
                ev.x = static_cast<unsigned short>(x);
                ev.y = static_cast<unsigned short>(y);
            }
...
int main(int argc, char *argv[]) {
    ...
    options_desc.add_options()
            ...
            ("input-path,i", po::value<std::filesystem::path>(&in_path)->required(),
             "Path to input event file or folder.")
            ("output-path,o", po::value<std::filesystem::path>(&out_path)->default_value(""),
             "Path to output file or folder. If not specified, will use the same as input.")
            ("recursive,r", po::bool_switch(&recursive_mode),
             "If specified, iterate over all files in the specified folder and sub-folders.")
            ("filename-pattern,p", po::value<std::string>(&filename_pattern),
             "Regex to match filenames to be converted, required in recursive mode.")
    ...
    if (!recursive_mode) {
        ...
        if (out_path.empty()) {
            out_path = in_path;
            out_path.replace_extension("_undist.hdf5");
        }
