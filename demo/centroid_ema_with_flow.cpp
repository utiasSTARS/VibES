...
int main(int argc, char *argv[]) {
    ...
    camera.cd().add_callback([&](const Metavision::EventCD *begin, const Metavision::EventCD *end) {
        ...
        for (auto it = begin; it != end; ++it) {
            ...
            if (vis_x_time >= vis_width) {
                ...
                cv::imwrite("/home/viciopoli/STARS/courses/centroid_ema_visualization_compensated_full.png",
                            compensated_frame);
    ...
    if (do_plot) {
        while (camera.is_running()) {
            ...
            cv::Mat frame_to_show_compensated_space;
            {
                std::lock_guard<std::mutex> lock(_mtx);
                ...
                compensated_frame.copyTo(frame_to_show_compensated_space);
//                crop_vis.setTo(cv::Scalar(0, 0, 0));
            ...
            if (!frame_to_show_compensated_space.empty()) {
                cv::imshow("Compensated Events Full", frame_to_show_compensated_space);
            }
            if (cv::waitKey(1) >= 0) {
                break;
            }
