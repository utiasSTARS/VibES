...
//    params.camera.biases().set_from_file("/home/viciopoli/Documents/metavision/biases/biases_filtered.bias");
    ...
    int visualization_cut_off = int(width * 0.99); // Cut-off for visualization
#endif
...
//        std::cout<<"Number of events processed: " << compensated_events.size() << std::endl;
    ...
    std::string video_out_path = params.params->output_folder + "/run_visualization.avi";

// Initialize VideoWriter
// Parameters: filename, fourcc codec, fps, frame size, isColor
    cv::VideoWriter video_writer;
    int fourcc = cv::VideoWriter::fourcc('X', 'V', 'I', 'D');
    double video_fps = 30.0; // Standard playback speed
    cv::Size frame_size(width, height);

    video_writer.open(video_out_path, fourcc, video_fps, frame_size, true);

    if (!video_writer.isOpened()) {
        std::cerr << "\033[1;31mCould not open the output video file for write\033[0m" << std::endl;
    } else {
        std::cout << "\033[1;32mRecording video to: " << video_out_path << "\033[0m" << std::endl;
    }
    ...
    if (video_writer.isOpened()) {
        ...
        std::cout << "\033[1;34mVideo saved successfully.\033[0m" << std::endl;
    }
