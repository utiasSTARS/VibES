...
class BinaryEdgeRegistration:
    ...
    def phase_correlation_registration(self, moving_img: np.ndarray,
                                       fixed_img: np.ndarray) -> RegistrationResult:
        """
        ...
        try:
            ...
            if self.phase_corr_params['hann_window']:
                hann_2d = np.outer(np.hanning(moving_float.shape[0]),
                                   np.hanning(moving_float.shape[1]))
            ...
            registered_img = cv2.warpAffine(moving_img, transform_matrix,
    ...
    def feature_based_registration(self, moving_img: np.ndarray,
                                   fixed_img: np.ndarray) -> RegistrationResult:
        """
        ...
        try:
            ...
            corners_moving = cv2.goodFeaturesToTrack(
                moving_img, **self.feature_params
            )
            corners_fixed = cv2.goodFeaturesToTrack(
                fixed_img, **self.feature_params
            )
            ...
            def extract_patch_descriptors(img, corners, patch_size=16):
                ...
                for corner in corners:
                    ...
                    # Check bounds
                    if (patch_size//2 <= x < img.shape[1] - patch_size//2 and
                            patch_size//2 <= y < img.shape[0] - patch_size//2):
            ...
            desc_moving, corners_moving_valid = extract_patch_descriptors(moving_img, corners_moving)
            desc_fixed, corners_fixed_valid = extract_patch_descriptors(fixed_img, corners_fixed)
            ...
            for i, desc_m in enumerate(desc_moving):
                ...
                if best_match_idx >= 0 and best_score > 0.7:  # Threshold for good matches
    ...
    def ecc_registration(self, moving_img: np.ndarray,
                         fixed_img: np.ndarray) -> RegistrationResult:
        """
        ...
        try:
            ...
            try:
                cc, warp_matrix = cv2.findTransformECC(
                    ...
                    self.ecc_params['motion_type'], criteria
                )
            except cv2.error:
                ...
                cc, warp_matrix_pyr = cv2.findTransformECC(
                    fixed_pyr, moving_pyr, warp_matrix,
                    self.ecc_params['motion_type'], criteria
                )
            ...
            # Apply transformation
            registered_img = cv2.warpAffine(moving_img, warp_matrix,
                                            (fixed_img.shape[1], fixed_img.shape[0]))
    ...
    def template_matching_registration(self, moving_img: np.ndarray,
                                       fixed_img: np.ndarray) -> RegistrationResult:
        """
    ...
    def register_image_pair(self, moving_img: np.ndarray,
                            fixed_img: np.ndarray) -> RegistrationResult:
        """
        ...
        if self.registration_method == 'auto':
            ...
            for method in methods_to_try:
                ...
                if result.success and (best_result is None or
                                       result.registration_score > best_result.registration_score):
                    best_result = result
            ...
            return best_result if best_result else RegistrationResult(
                transformation_matrix=np.eye(2, 3, dtype=np.float32),
                registered_image=moving_img.copy(),
                registration_score=0.0,
        ...
        if self.registration_method in method_map:
            return method_map[self.registration_method](moving_processed, fixed_processed)
        else:
...
def register_image_sets(gt_folder: str, sev_folder: str, amiev_folder: str,
                        output_folder: str = 'registered_outputs',
                        registration_method: str = 'auto') -> Dict:
    """
    ...
    common_names = set(gt_images.keys()) & set(sev_images.keys()) & set(amiev_images.keys())
    ...
    for img_name in sorted(common_names):
        ...
        if sev_result.success:
            cv2.imwrite(str(output_path / 'registered_sev' / f'{img_name}.png'),
        ...
        if amiev_result.success:
            cv2.imwrite(str(output_path / 'registered_amiev' / f'{img_name}.png'),
        ...
        create_registration_visualization(
            ...
            sev_result, amiev_result
        )

        results['statistics']['total_processed'] += 1

    # Calculate final statistics
    results['statistics']['average_sev_score'] = np.mean(sev_scores) if sev_scores else 0.0
    results['statistics']['average_amiev_score'] = np.mean(amiev_scores) if amiev_scores else 0.0
...
def create_registration_visualization(gt_img, sev_orig, amiev_orig,
                                      sev_reg, amiev_reg, output_path,
                                      sev_result, amiev_result):
...
def save_registration_report(results, output_path):
    ...
    with open(output_path, 'w') as f:
        ...
        f.write(f"Average AMI-EV registration score: {stats['average_amiev_score']:.4f}\n\n")
        ...
        for img_name in sorted(results['sev_registrations'].keys()):
            ...
            f.write(f"\nImage: {img_name}\n")
            f.write(f"  S-EV: {sev_res.method_used} | Score: {sev_res.registration_score:.4f} | Success: {sev_res.success}\n")
            f.write(f"  AMI-EV: {amiev_res.method_used} | Score: {amiev_res.registration_score:.4f} | Success: {amiev_res.success}\n")

def calculate_ods_f1_score(gt_edges: np.ndarray, pred_edges: np.ndarray,
...
if __name__ == "__main__":
    ...
    try:
        ...
        if 'statistics' in results:
            ...
            print(f"S-EV success rate: {stats['sev_successful']}/{stats['total_processed']}")
            print(f"AMI-EV success rate: {stats['amiev_successful']}/{stats['total_processed']}")
